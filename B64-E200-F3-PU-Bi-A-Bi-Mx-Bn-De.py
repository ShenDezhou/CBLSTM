...
from keras.layers import Dense, Embedding, RepeatVector, add, Add, SpatialDropout1D, Conv1D,MaxPooling1D, AveragePooling1D, Input, Bidirectional, Flatten,CuDNNLSTM, Lambda, Dropout, BatchNormalization, Maximum, concatenate,Reshape, Concatenate
...
#
#            P     0.6349    0.7473    0.6865       463
#            N     0.8206    0.7289    0.7720       734
#
#    micro avg     0.7360    0.7360    0.7360      1197
#    macro avg     0.7277    0.7381    0.7293      1197
# weighted avg     0.7487    0.7360    0.7389      1197
...
if MODE == 1:
    with codecs.open('plain/ctrip_training.utf8', 'r', encoding='utf8') as ft:
        with codecs.open('plain/ctrip_training_states.utf8', 'r', encoding='utf8') as fs:
            with codecs.open('model/f3/ctrip_train_crffeatures.pkl', 'wb') as fx:
                with codecs.open('model/f3/ctrip_train_crfstates.pkl', 'wb') as fy:
                    xlines = ft.readlines()
                    ylines = fs.readlines()
                    X = []
                    y = []
...
if MODE==2:
    ...
    # embeddedd = Embedding(len(bigrams) + 1, word_size, embeddings_initializer=Constant(zhwiki_biemb),input_length=maxlen,
    #                       mask_zero=False)(seqsd)
    # embeddede = Embedding(len(bigrams) + 1, word_size, embeddings_initializer=Constant(zhwiki_biemb),input_length=maxlen,
    ...
    predropout = SpatialDropout1D(rate=ODropoutrate)(sumbigram)
    # conv = Conv1D(filters=nfilters, kernel_size=kernelSize, activation='relu')(predropout)

    blstm = Bidirectional(CuDNNLSTM(Hidden, batch_input_shape=(maxlen, nFeatures), return_sequences=True), merge_mode='concat')(predropout)

    avgp = AveragePooling1D(pool_size=poolSize)(blstm)
    maxp = MaxPooling1D(pool_size=poolSize)(blstm)
    concatp = concatenate([avgp, maxp])
    innerdropout = SpatialDropout1D(rate=TDropoutrate)(concatp)

    postblstm = Bidirectional(CuDNNLSTM(Hidden, batch_input_shape=(maxlen, nFeatures), return_sequences=False), merge_mode='concat')(concatp)

    postdropout = Dropout(rate=FDropoutrate)(postblstm)


    batchNorm = BatchNormalization()(postdropout)
    ...
    with codecs.open('model/f3/ctrip_train_crffeatures.pkl', 'rb') as fx:
        with codecs.open('model/f3/ctrip_train_crfstates.pkl', 'rb') as fy:
            with codecs.open('model/ctrip_train_%s_model.pkl'%modelfile, 'wb') as fm:
                ...
                # with open("keras/B20-E60-F10-PU-Bi-RCT-CT-Bn-De.json", "w") as json_file:
