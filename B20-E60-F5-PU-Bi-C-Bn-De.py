...
if MODE==2:
    ...
    dropout = SpatialDropout1D(rate=Dropoutrate)(sumbigram)
    ...
    conv = Conv1D(nb_filter=nfilters, kernel_size=kernelSize, activation='relu')(batchNorm)
    flat = Flatten()(conv)
    dense = Dense(nState, activation='softmax', kernel_regularizer=regularizers.l2(Regularization))(flat)
    ...
    with codecs.open('model/f5/ctrip_train_crffeatures.pkl', 'rb') as fx:
        with codecs.open('model/f5/ctrip_train_crfstates.pkl', 'rb') as fy:
            with codecs.open('model/ctrip_train_B20-E60-F5-PU-Bi-C-Bn-De_model.pkl', 'wb') as fm:
                bx = fx.read()
                by = fy.read()
                X = pickle.loads(bx)
                y = pickle.loads(by)
                ...
                model.save("keras/B20-E60-F5-PU-Bi-C-Bn-De.h5")
                print('FIN')

if MODE == 3:
    STATES = list("PN")
    with codecs.open('plain/ctrip_test.utf8', 'r', encoding='utf8') as ft:
        with codecs.open('baseline/ctrip_test_B20-E60-F5-PU-Bi-C-Bn-De_states.txt', 'w', encoding='utf8') as fl:
            with codecs.open('model/ctrip_train_B20-E60-F5-PU-Bi-C-Bn-De_model.pkl', 'rb') as fm:
                # bm = fm.read()
                # model = pickle.loads(bm)
                model = load_model("keras/B20-E60-F5-PU-Bi-C-Bn-De.h5")
                model.summary()
