...
#
#            P     0.7780    0.7041    0.7392       463
#            N     0.8239    0.8733    0.8479       734
#
#    micro avg     0.8079    0.8079    0.8079      1197
#    macro avg     0.8010    0.7887    0.7936      1197
# weighted avg     0.8062    0.8079    0.8059      1197
...
words = []
with codecs.open('ctrip_dic/ctrip_word.utf8', 'r', encoding='utf8') as f:
    lines = f.readlines()
    for line in lines:
        line = line.strip()
        if len(line) > 0:
...
rwxdict = dict(zip(words, range(1, 1+len(words))))
rwxdict['\n'] = 0
...
def getWordVector(sentence, i):
    ngrams = getWord(sentence, i)
    ngramv = []
    for ngram in ngrams:
        if len(ngram)==3:
            if '\n' in ngram:
                ...
                ngramv.append(rwxdict.get(ngram, 0))
    return ngramv
...
def getFeatures(sentence, i):
    ...
    features.extend(getWordVector(sentence, i))
...
if MODE == 1:
    with codecs.open('plain/ctrip_training.utf8', 'r', encoding='utf8') as ft:
        with codecs.open('plain/ctrip_training_states.utf8', 'r', encoding='utf8') as fs:
            with codecs.open('model/f6/ctrip_train_crffeatures.pkl', 'wb') as fx:
                with codecs.open('model/f6/ctrip_train_crfstates.pkl', 'wb') as fy:
                    xlines = ft.readlines()
                    ylines = fs.readlines()
                    X = []
                    y = []
...
if MODE==2:
    ...
    zhwiki_wemb = sparse.load_npz("model/zhwiki_wembedding.npz").todense()
    embeddedf = Embedding(len(words) + 1, word_size, embeddings_initializer=Constant(zhwiki_wemb),
                          input_length=maxlen,
    ...
    with codecs.open('model/f6/ctrip_train_crffeatures.pkl', 'rb') as fx:
        with codecs.open('model/f6/ctrip_train_crfstates.pkl', 'rb') as fy:
