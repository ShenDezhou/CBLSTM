...
from keras.layers import Dense, Embedding, RepeatVector, add, Add, SpatialDropout1D, Conv1D,MaxPooling1D, Input, Bidirectional, Flatten,CuDNNLSTM, Lambda, Dropout, BatchNormalization, Maximum, concatenate,Reshape, Concatenate
...
#
#            P     0.7593    0.6199    0.6825       463
#            N     0.7851    0.8760    0.8281       734
#
#     accuracy                         0.7769      1197
#    macro avg     0.7722    0.7479    0.7553      1197
# weighted avg     0.7751    0.7769    0.7718      1197
...
if MODE == 1:
    with codecs.open('plain/ctrip_training.utf8', 'r', encoding='utf8') as ft:
        with codecs.open('plain/ctrip_training_states.utf8', 'r', encoding='utf8') as fs:
            with codecs.open('model/f5/ctrip_train_crffeatures.pkl', 'wb') as fx:
                with codecs.open('model/f5/ctrip_train_crfstates.pkl', 'wb') as fy:
                    xlines = ft.readlines()
                    ylines = fs.readlines()
                    X = []
                    y = []
...
if MODE==2:
    ...
    predropout = SpatialDropout1D(rate=Dropoutrate)(sumbigram)
    conv = Conv1D(nfilters,kernelSize)(predropout)

    blstm = Bidirectional(CuDNNLSTM(Hidden, batch_input_shape=(maxlen, nFeatures), return_sequences=False), merge_mode='sum')(conv)
    # dropout = Dropout(rate=Dropoutrate)(blstm)
    postdropout = SpatialDropout1D(rate=Dropoutrate)(blstm)
    ...
    with codecs.open('model/f5/ctrip_train_crffeatures.pkl', 'rb') as fx:
        with codecs.open('model/f5/ctrip_train_crfstates.pkl', 'rb') as fy:
            with codecs.open('model/ctrip_train_B20-E60-F5-PU-C-Bi-Bn-De_model.pkl', 'wb') as fm:
                bx = fx.read()
                by = fy.read()
                X = pickle.loads(bx)
                y = pickle.loads(by)
                ...
                model.save("keras/B20-E60-F5-PU-C-Bi-Bn-De.h5")
                print('FIN')

if MODE == 3:
    STATES = list("PN")
    with codecs.open('plain/ctrip_test.utf8', 'r', encoding='utf8') as ft:
        with codecs.open('baseline/ctrip_test_B20-E60-F5-PU-C-Bi-Bn-De_states.txt', 'w', encoding='utf8') as fl:
            with codecs.open('model/ctrip_train_B20-E60-F5-PU-C-Bi-Bn-De_model.pkl', 'rb') as fm:
                # bm = fm.read()
                # model = pickle.loads(bm)
                model = load_model("keras/B20-E60-F5-PU-C-Bi-Bn-De.h5")
                model.summary()
